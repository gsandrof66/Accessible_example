...
ui <- fluidPage(
  ...
  titlePanel(textOutput("dynamicTitle")),
  selectInput("theme", "Select Theme:", choices = c("vapor", "flatly"), selected = "flatly"),
  
  # Navigation Bar
  navbarPage("My App",
             tabPanel("Home", fluidPage(h2("Welcome to Home"))),
             tabPanel("Data", fluidPage(h2("Data Section"), tableOutput("data_table"))),
             tabPanel("Maps", fluidPage(h2("Map review"))),
             tabPanel("Settings", fluidPage(h2("App Settings")))
  )
